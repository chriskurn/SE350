...
final public class Building {
	...
	public void enterFloor(Person p, int floor) throws IllegalParamException, InvalidFloorException{
	    if(floor > this.getNumberOfFloors() || floor <= 0){
	        throw new InvalidFloorException("The floor exceeds the number of floors of this building.");
	    }else{
	        this.getMyFloors().get(floor).enterFloor(p);
	    } 
	}
	...
	private void buildFloors(){
	    ...
	    Simulator.getInstance().logEvent(String.format("Building %d number of floors.",numFloors));
